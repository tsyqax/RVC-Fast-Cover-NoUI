...
bh, ah = signal.butter(N=5, Wn=48, btype="높음", fs=16000)
...
class VC(object):
    ...
    def vc(
        ...
        expected_output_len = p_len * 256
        audio1 = audio1[:expected_output_len]
    ...
    def pipeline(
        ...
        for t in opt_ts:
            ...
            current_p_len = (t + self.t_pad2) // self.window - s // self.window
            if if_f0 == 1:
                audio_opt.append(
                    self.vc(
                        model,
            ...
            s = t
        if if_f0 == 1:
            last_p_len = p_len - (t // self.window if t is not None else 0)
            audio_opt.append(
                self.vc(
                    model,
                    net_g,
